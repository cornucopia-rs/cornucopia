...
pub async fn main() {
    ...
    let authors = authors(&client).bind().all().await.unwrap();
    dbg!(authors);
    ...
    // Queries also accept transactions. Let's see how that works.
    {
        ...
        insert_book(&tx).bind(&"The Great Gatsby").await.unwrap();
    ...
    let author_name = author_name_by_id(&client).bind(&0).opt().await.unwrap();
    dbg!(author_name);
    ...
    let patrick_voice_actor = select_voice_actor_with_character(&client)
    ...
    let translations = select_translations(&client)
        .bind()
