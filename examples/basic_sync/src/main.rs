...
pub fn main() {
    ...
    let authors = authors(&mut client).bind().all().unwrap();
    dbg!(authors);
    ...
    // Queries also accept transactions. Let's see how that works.
    {
        ...
        insert_book(&mut tx).bind(&"The Great Gatsby").unwrap();
        ...
        // for more details.
        insert_book(&mut tx)
            .bind(&String::from("Moby Dick"))
            .unwrap();

        // You can use a `map` to transform query results ergonomically.
        let uppercase_books = books(&mut tx)
            .bind()
    ...
    let author_name = author_name_by_id(&mut client).bind(&0).opt().unwrap();
    dbg!(author_name);
    ...
    let name_starting_with_jo = author_name_starting_with(&mut client)
        .params(&AuthorNameStartingWithParams { start_str: "Jo" })
        .all()
        .unwrap();
    ...
    let patrick_voice_actor = select_voice_actor_with_character(&mut client)
        .bind(&SpongeBobCharacter::Patrick)
        .one()
        .unwrap();
    ...
    let translations = select_translations(&mut client)
        .bind()
