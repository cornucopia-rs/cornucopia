...
fn gen_row_structs(
    ...
    if *is_named {
        ...
        if *is_copy {
            gen!(
                w,
                "impl cornucopia_{client_name}::Borrow for {name} {{
                    type Borrow<'r> = {name};
                }}"
            );
        } else {
...
fn gen_query_fn(
    ...
    if let Some((idx, index)) = row {
        ...
        gen!(w,
            "pub fn bind<'a, C: GenericClient,{generic}>(&'a mut self, client: &'a {client_mut} C, {param_list}) -> cornucopia_{client_name}::private::Query<'a,C, {row_struct_name}, {row_struct_name}, {nb_params}> {{
                cornucopia_{client_name}::private::Query {{
                    client,
                    params: [{param_names}],
                    stmt: &mut self.0,
    ...
    if let Some(param) = param {
        ...
        if param.is_named {
            ...
            if let Some((idx, _)) = row {
                ...
                let nb_params = param_field.len();
                gen!(w,"impl <'a, C: GenericClient,{generic}> cornucopia_{client_name}::Params<'a, {param_name}<{lifetime}{traits}>, cornucopia_{client_name}::private::Query<'a, C, {query_row_struct}, {query_row_struct}, {nb_params}>, C> for {struct_name}Stmt {{ 
                    fn params(&'a mut self, client: &'a {client_mut} C, params: &'a {param_name}<{lifetime}{traits}>) -> cornucopia_{client_name}::private::Query<'a, C, {query_row_struct}, {query_row_struct}, {nb_params}> {{
    ...
    match content {
        PreparedContent::Enum(variants) => {
            ...
            gen!(w,
                    ...
                    impl cornucopia_{client_name}::Borrow for {struct_name} {{
